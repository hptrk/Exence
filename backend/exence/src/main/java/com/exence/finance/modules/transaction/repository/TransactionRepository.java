...
public interface TransactionRepository
    ...
    @Query("SELECT t FROM Transaction t WHERE " + "t.recurring = true "
            + "AND t.type = :type "
            + "ORDER BY t.date DESC")
    Page<Transaction> findRecurringByType(@Param("type") TransactionType type, Pageable pageable);

    @Query("SELECT t FROM Transaction t WHERE " + "t.recurring = true " + "ORDER BY t.date DESC")
