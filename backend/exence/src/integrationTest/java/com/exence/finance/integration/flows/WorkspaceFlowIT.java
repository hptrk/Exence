...
class WorkspaceFlowIT extends BaseFlowIT {
    ...
    void flowWs01_workspaceCrudAndLastWorkspaceProtection() {
        ...
        // 3. DELETE last workspace → 403 WORKSPACE_CANNOT_DELETE_LAST
        workspaceActor()
                .deleteWorkspaceRaw(user, ws1Id)
                .statusCode(403)
                .body("code", equalTo("workspace-cannot-delete-last"));

        // 4. POST /workspaces → 201, second workspace
        var ws2 = workspaceActor()
                .createWorkspace(user, ITFixtures.workspace().name("Second WS").build());
        ...
        var renamed = workspaceActor()
                .renameWorkspace(
                        user,
                        ws2.id(),
                        ITFixtures.workspaceRename().name("Renamed WS").build());
        ...
        // 9. PATCH non-existent workspace → 403 WORKSPACE_NOT_FOUND
        workspaceActor()
                .renameWorkspaceRaw(
                        user.withWorkspace(ws1Id),
                        ws1Id,
                        ITFixtures.workspaceRename().build())
                .statusCode(403)
    ...
    void flowWs03_memberInviteLeaveAndRemoveLifecycle() {
        ...
        assertThat(workspaceActor().listWorkspaces(userB).stream().anyMatch(ws -> ws.id() == wsId))
                .isTrue();
        ...
        assertThat(workspaceActor().listWorkspaces(userB).stream().noneMatch(ws -> ws.id() == wsId))
                .isTrue();
        ...
        // 11. UserA (owner): leave own workspace → 403 WORKSPACE_OWNER_CANNOT_LEAVE
        workspaceActor()
                .leaveWorkspaceRaw(userA, wsId)
                .statusCode(403)
        ...
        workspaceActor()
                .removeMemberRaw(
                        userA,
                        wsId,
                        new WorkspaceMemberEmailRequest(userA.user().email()))
                .statusCode(403)
