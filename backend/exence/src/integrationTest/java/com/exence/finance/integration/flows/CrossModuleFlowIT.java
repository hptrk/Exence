...
class CrossModuleFlowIT extends BaseFlowIT {
    ...
    void flowCross01_fullFinancialOnboarding() {
        ...
        var catExpense = categoryActor()
                .createCategory(
                        user, ITFixtures.expenseCategory().name("Restaurant").build());
        var catIncome = categoryActor().createIncomeCategory(user);

        // 7-8. Create transactions
        var txn1 = transactionActor()
                .createTransaction(
                        user,
        ...
        var goal = goalActor()
                .createGoal(
                        user,
                        ITFixtures.goal(catExpense.id())
                                .targetAmount(new BigDecimal("100000"))
                                .currentAmount(BigDecimal.ZERO)
                                .build());
        var partialGoal = goalActor()
                .patchGoal(
                        user,
                        goal.id(),
        ...
        var debt = debtActor()
                .createDebt(
                        user,
                        ITFixtures.debt(catExpense.id())
                                .amount(new BigDecimal("20000"))
                                .build());
        var paidDebt = debtActor()
                .payDebt(
                        user,
                        debt.id(),
    ...
    void flowCross02_workspaceCollaborationAndVisibility() {
        ...
        workspaceActor()
                .addMember(
                        userA,
                        wsAId,
                        new WorkspaceMemberEmailRequest(userB.user().email()));
        ...
        var catA = categoryActor()
                .createCategory(
                        userA,
                        ITFixtures.expenseCategory().name("Shared category").build());

        // 6. UserB sees category in Workspace-A
        assertThat(categoryActor().listCategories(userBInWsA).stream().anyMatch(c -> c.id().equals(catA.id())))
                .isTrue();

        // 7. UserB creates transaction in Workspace-A
        transactionActor()
                .createTransaction(userBInWsA, ITFixtures.transaction(catA.id()).build());
        ...
        categoryActor().getCategoryRaw(userBInWsA, catA.id()).statusCode(403);
    }
    ...
    void flowCross04_recurringTransactionGoalTracking() {
        ...
        assertThat(transactionActor().getRecurring(user, rec.id()).nextExecutionDate())
                .isNotNull();

        // 6. Goal currentAmount is still 0
        assertThat(goalActor().getGoal(user, savingsGoal.id()).currentAmount()).isEqualByComparingTo(BigDecimal.ZERO);

        // 7-8. Simulate first monthly contribution
        var after1 = goalActor()
                .patchGoal(
                        user,
                        savingsGoal.id(),
                        ITFixtures.goalPatch()
        ...
        // 9. Complete goal
        var goalDone = goalActor()
                .patchGoal(
                        user,
                        savingsGoal.id(),
                        ITFixtures.goalPatch()
                                .currentAmount(new BigDecimal("120000"))
                                .build());
    ...
    void flowCross05_achievementTriggeringAcrossModules() {
        ...
        var debt = debtActor()
                .createDebt(
                        user,
                        ITFixtures.debt(cat.id()).amount(new BigDecimal("500")).build());
        debtActor()
                .payDebt(
                        user,
                        debt.id(),
    ...
    void flowCross06_preAccountDeletionDataCheck() {
        ...
        transactionActor()
                .createTransaction(
                        user, ITFixtures.transaction(cat.id()).title("T1").build());
        transactionActor()
                .createTransaction(
                        user, ITFixtures.transaction(cat.id()).title("T2").build());
        transactionActor()
                .createTransaction(
                        user, ITFixtures.transaction(cat.id()).title("T3").build());
    ...
    void flowCross07_workspaceHeaderValidations() {
        ...
        categoryActor()
                .createCategoryWithoutWorkspaceHeaderRaw(
                        user, ITFixtures.expenseCategory().build())
                .statusCode(400);

        // 3. POST /categories with non-existent workspace ID → 403 WORKSPACE_NOT_FOUND
        categoryActor()
                .createCategoryRaw(
        ...
        categoryActor()
                .listCategoriesRaw(user.withWorkspace(userB.workspaceId()))
                .statusCode(403);
