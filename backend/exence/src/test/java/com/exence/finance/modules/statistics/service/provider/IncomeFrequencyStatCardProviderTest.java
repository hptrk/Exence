...
class IncomeFrequencyStatCardProviderTest {
    ...
    @InjectMocks
    private IncomeFrequencyStatCardProvider provider;

    @Test
    @DisplayName("returns INCOME_FREQUENCY_STATCARD as the supported widget type")
    void getSupportedType_returnsIncomeFrequencyStatCard() {
        assertThat(provider.getSupportedType()).isEqualTo(StatisticsWidgetType.INCOME_FREQUENCY_STATCARD);
    }
    ...
    void getData_withResults() {
        ...
        StatCardPayload expected = new StatCardPayload(
                StatisticsWidgetType.INCOME_FREQUENCY_STATCARD, BigDecimal.TEN, null, null, null, null, null, null);
